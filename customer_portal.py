...
def _filter_for_customer_allergies(products, customer):
    if not customer:
    ...
    lowered_allergies = [a.lower().strip() for a in allergies if a]
    ...
    for p in products:
        ...
        prod_allergens_lower = [a.lower().strip() for a in prod_allergens]
        if any(a in prod_allergens_lower for a in lowered_allergies):
            hidden_count += 1
            continue
        safe_products.append(p)
    return safe_products, hidden_count > 0

def _render_menu(supabase):
    st.subheader("Menu")
    st.caption("Browse available items and add them to your order. Items that contain your listed allergens are automatically hidden.")

    with st.container():
        col_f1, col_f2 = st.columns([2, 1])
        with col_f1:
            search = st.text_input("Search products", placeholder="Search by name or description...", key="search_products")
        with col_f2:
            st.write("Dietary filters")
            active_filters = []
            cols = st.columns(len(DIETARY_FILTERS))
            for (label, key_name), col in zip(DIETARY_FILTERS.items(), cols):
                if col.checkbox(label, value=False, key=f"flt_{key_name}"):
                    active_filters.append(key_name)
    ...
    # Apply allergy-based filtering for logged-in customers
    customer = get_current_customer()
    products, hid_any = _filter_for_customer_allergies(products, customer)
    if hid_any:
        st.info("Some items have been hidden because they contain allergens you have listed in your profile.")

    if not products:
        st.info("No products found. Please adjust filters or your allergy settings.")
        return {}
...
def render_customer_portal(supabase):
    ...
    last_cutoff_raw = slot_rules.get("last_same_day_order_time") or "15:00"

    # Supabase REST often returns TIME columns as strings like "15:00:00"
    # and sometimes as "15:00". Handle both safely.
    if isinstance(last_cutoff_raw, str):
        s = last_cutoff_raw.strip()
        try:
            cutoff_time = datetime.strptime(s, "%H:%M").time()
        except ValueError:
            cutoff_time = datetime.strptime(s, "%H:%M:%S").time()
    else:
        cutoff_time = last_cutoff_raw
    ...
    if not allow_today and date_choice == now.date():
        ...
        if slot_labels:
            slot_label = st.selectbox("Available time slots", [l[0] for l in slot_labels], key="collection_slot")
        else:
    ...
    with col1:
        name = st.text_input("Name", value=default_name, key="checkout_name")
        email = st.text_input("Email", value=default_email, key="checkout_email")
    with col2:
        phone = st.text_input("Phone number", value=default_phone, key="checkout_phone")
        order_type = st.selectbox("Order type", ["Pickup", "Delivery"], key="checkout_order_type")

    address = default_address
    if order_type == "Delivery":
        address = st.text_area("Delivery address (Wiveliscombe only)", value=default_address, key="checkout_address")
    ...
    tcs = st.checkbox("I accept the terms and conditions", key="checkout_tcs")
...
"""
    ...
    with col_right:
        place_order = st.button("Place order", type="primary", use_container_width=True, key="checkout_place_order")
    ...
    if place_order:
        ...
        order_payload = {
            ...
            "delivery_address_snapshot": address if order_type == "Delivery" else "",
        }
...
def render_order_tracking(supabase):
    ...
    code = st.text_input("Order number (e.g. ORD-123456)", key="track_order_code")
    if st.button("Check status", key="track_order_button"):
